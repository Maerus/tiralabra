...
public class Edge {
    ...
    private final int FLOORWEIGHT = 1;
    private final int WALLWEIGHT = 10000;

    /**
     * Constructs a new edge between two nodes and assigns a weight depending on
     * if either vertex is a wall tile
     *
        ...
        if (v1.getTileType() != null && v2.getTileType() != null) {
            weight = v1.getTileType().equals(TileType.WALL)
                    || v2.getTileType().equals(TileType.WALL)
                            ? WALLWEIGHT : FLOORWEIGHT;
        } else if (v1.getTileType() != null) {
            weight = v1.getTileType().equals(TileType.WALL) ? WALLWEIGHT : FLOORWEIGHT;
        } else if (v2.getTileType() != null) {
            weight = v2.getTileType().equals(TileType.WALL) ? WALLWEIGHT : FLOORWEIGHT;
        } else {
            weight = WALLWEIGHT;
        }
